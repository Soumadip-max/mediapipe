...
Respond with VALID JSON ONLY. Do not wrap in markdown code blocks or additional prose.`;
        ...
        try {
            ...
            console.warn("Primary gemini-1.5-flash failed, attempting fallback model:", modelErr);
            try {
                const response = await ai.models.generateContent({
        ...
        return NextResponse.json(getFallbackRoadmap("Engineering", "mid"));
    }
}

function getFallbackRoadmap(domain: string, level: string) {
    return {
        roleTitle: `${domain} Roadmap`,
        overview: `Comprehensive interactive pathway for mastering ${domain} at ${level} level.`,
        nodes: [
            {
                id: "1",
                label: `${domain} Core Fundamentals & Architecture`,
                phase: "Phase 1: Core Primitives",
                position: { x: 0, y: 0 },
                type: "main",
                description: `Master core runtime mechanics, language execution models, and design patterns for ${domain}.`,
                keyConcepts: "Language Primitives, Execution Model, Async Event Loop, Memory Management",
                notes: "Focus on non-blocking I/O and low-latency execution paths.",
                youtubeQuery: `${domain} core architecture tutorial`,
                status: "learning",
            },
            {
                id: "2",
                label: "Framework & State Orchestration",
                phase: "Phase 2: Framework Mastery",
                position: { x: -260, y: 220 },
                type: "subtopic",
                description: "Design reactive state flows, client/server boundaries, and caching layers.",
                keyConcepts: "State Machines, Caching Strategies, Data Serialization",
                notes: "Prevent unnecessary synchronization cascades.",
                youtubeQuery: `${domain} state management design patterns`,
                status: "learning",
            },
            {
                id: "3",
                label: "Data Access & High-Performance Storage",
                phase: "Phase 2: Framework Mastery",
                position: { x: 260, y: 220 },
                type: "subtopic",
                description: "Optimize database schemas, indexes, query plans, and in-memory caches.",
                keyConcepts: "B-Tree Indexes, Connection Pools, Redis Invalidation, Query Plans",
                notes: "Benchmark throughput and avoid N+1 query patterns.",
                youtubeQuery: `${domain} database query optimization tutorial`,
                status: "learning",
            },
            {
                id: "4",
                label: "Production Cloud Infrastructure & Telemetry",
                phase: "Phase 3: Production Scale",
                position: { x: 0, y: 440 },
                type: "main",
                description: "Deploy automated CI/CD pipelines, container orchestration, and distributed tracing.",
                keyConcepts: "Docker, Kubernetes, OpenTelemetry, Observability, CI/CD",
                notes: "Configure automated canary deployments and p99 alerts.",
                youtubeQuery: `${domain} production deployment observability tutorial`,
                status: "learning",
            },
        ],
        edges: [
